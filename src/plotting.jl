...
"""
function _resolve_cols(data :: EigenphData; cols=:all, selector=nothing)
...
- `label=:name` uses data.name[col] as the legend label
"""
function plot_eigenph(
  ...
  colidx = _resolve_cols(data; cols=cols, selector=selector)
  isempty(colidx) && error("No columns selected")
  plt = Plots.plot(; kwargs...)
  for c in colidx
    lbl = label === :name ? data.name[c] :
...
function plot_records(
  ...
  for r in records
    for c in colidx
      lbl = "$(record_label(r)) | $(r.data.name[c])"
